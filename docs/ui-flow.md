...
sequenceDiagram
    ...
    Auth->>User: 認証要求
    User->>Auth: Google アカウント認証
    Auth->>Settings: 基本設定画面
    Settings->>User: APIキー・言語設定
    User->>Settings: 設定入力・保存
...
sequenceDiagram
    ...
    Main->>User: メイン画面表示
    User->>Main: "録音開始" ボタン
    Main->>Recording: 録音中画面に切り替え
    Recording->>User: 録音状態・進捗表示
    
    Note over Recording: リアルタイム文字起こし表示
